...
if(!empty($items)&&is_array($items))
{
    ...
    foreach ($items as $item)
    {
        ...
        <tr<?php if(!(int)$item['active']){?> class="bg-warning"<?php };?>>
            <td class="w-25 p-1 p-sm-2"><?=$item['username'];?></td>
            <td class="w-25 p-1 p-sm-2"><?=$item['email'];?></td>
            ...
            <td class="text-center p-1 p-sm-2"><?= Html::a('<i class="fas fa-edit"></i>', '/site-user-edit-form/'.$item['id'], ['class'=>'btn btn-secondary']); ?></td>
            <td class="text-center p-1 p-sm-2"><?= Html::a('<i class="fas fa-trash"></i>', '/site-user-delete/'.$item['id'], ['data-confirm'=>"Удалить запись?", 'class'=>'btn btn-danger']); ?></td>
        </tr>
        <?php
    }
    ?></table><?php
